...
export class AppComponent {
    ...
    public currentItem: Show | null = null;
    ...
    // Método para actualizar el show en la lista
    public updateElement(updatedShow: Show): void {
        if (this.currentItem) {
            const index = this.shows.findIndex(show => show.name === this.currentItem!.name);
            if (index !== -1) {
                this.shows[index] = updatedShow;  // Actualizamos el show
            }
        }
